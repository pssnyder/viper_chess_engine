...
class ChessGame:
    def __init__(self, fen_position=None):
        ...
        self.white_bot_type = self.config.get('white_ai_config', {}).get('ai_type', 'random')
        self.black_bot_type = self.config.get('black_ai_config', {}).get('ai_type', 'random')
        self.white_ai_type = self.white_bot_type
        self.black_ai_type = self.black_bot_type
        ...
        if self.logging_enabled and self.logger:
            ...
            self.logger.debug(f"White AI Type: {self.white_bot_type}, Engine: {self.white_eval_engine}")
            self.logger.debug(f"Black AI Type: {self.black_bot_type}, Engine: {self.black_eval_engine}")
    ...
    def process_ai_move(self):
        ...
        if not self.white_engine.sync_with_game_board(self.board):
            if self.logger:
        ...
        if not self.black_engine.sync_with_game_board(self.board):
            if self.logger:
                self.logger.error("Failed to synchronize black evaluation engine with game board.")
            return
        def stop_callback():
            return self.white_engine.has_game_board_changed() or self.black_engine.has_game_board_changed()
        ...
        try:
            self.current_player = chess.WHITE if self.board.turn else chess.BLACK
            if self.ai_vs_ai:
                # AI vs AI mode: use the appropriate engine based on the current player
            ...
            # Validate move against the current game board
            if isinstance(ai_move, chess.Move) and self.board.is_legal(ai_move):
                # If the move is legal, push it to the board and game node
                self.push_move(ai_move)
                self.last_ai_move = ai_move
            else:
                if self.logging_enabled and self.logger:
                    self.logger.warning(f"Could not process AI move, falling back on random legal move: {ai_move} invalid for {self.current_player}. | FEN: {self.board.fen()}")
            ...
            if self.logging_enabled and self.logger:
                self.logger.error(f"Error processing AI move: {e}")
            # Fallback: try a random legal move if possible
            legal_moves = list(self.board.legal_moves)
            if legal_moves:
                fallback = random.choice(legal_moves)
                ai_move = fallback
    ...
    def process_ai_move_threaded(self):
        ...
        self.current_player = chess.WHITE if self.board.turn else chess.BLACK
        if self.ai_busy or self.board.is_game_over(claim_draw=self.white_engine._is_draw_condition(self.board)) or not self.screen_ready:
            return
        self.ai_busy = True
        self.ai_move_result = None
    ...
    def push_move(self, move):
        ...
        try:
            ...
            if isinstance(move, str):
                ...
                if self.logging_enabled and self.logger:
                    self.logger.info(f"Converted move from string: {move}")
            else:
                move = chess.Move.from_uci(move.uci()) if hasattr(move, 'uci') else move
                if self.logging_enabled and self.logger:
                    self.logger.info(f"Using move object: {move}")
    ...
    def run(self):
        ...
        while running and ((self.ai_vs_ai and game_count >= 1)):
            if self.logging_enabled and self.logger:
                ...
                self.black_engine.logger.info(f"Running chess game loop: {self.game_count - game_count}/{self.game_count} remaining")
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.QUIT:
                    ...
                    if self.watch_mode() and not self.ai_busy and self.screen_ready and not self.board.is_game_over(claim_draw=self.white_engine._is_draw_condition(self.board)) and self.board.is_valid():
            ...
            if self.watch_mode and self.ai_move_result is not None and not self.ai_busy:
                ...
                if self.logging_enabled and self.logger:
                    ...
                    self.white_engine.logger.info("AI move took %d ms", move_duration)
                    self.black_engine.logger.info("AI move took %d ms", move_duration)
