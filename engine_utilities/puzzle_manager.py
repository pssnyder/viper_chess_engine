...
class PuzzleManager:
    ...
    def __init__(self, debug=False, db_path="puzzles.db", csv_path="puzzles.csv"):
        self.puzzles = []
        self.config = {}
        self.puzzle_config = {}
    ...
    def get_random_puzzle(self, min_rating=0, max_rating=9999, themes=None, max_moves=None):
        c = self.conn.cursor()
        query = "SELECT * FROM puzzles WHERE rating >= ? AND rating <= ?"
        params = [min_rating, max_rating]
        if themes:
            query += " AND (" + " OR ".join(["themes LIKE ?"] * len(themes)) + ")"
            params += [f"%{t.lower()}%" for t in themes]
        if max_moves:
            query += " AND (LENGTH(moves) - LENGTH(REPLACE(moves, ' ', '')) + 1) <= ?"
